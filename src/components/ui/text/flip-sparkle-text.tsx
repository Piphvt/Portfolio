...
const SparkleComp: React.FC<Sparkle> = ({ id, x, y, color, delay, scale }) => (
  <motion.svg
    key={id}
...
interface FlipSparkleTextProps extends MotionProps {
  children: ReactNode;
  className?: string;
  duration?: number;
...
export const FlipSparkleText: React.FC<FlipSparkleTextProps> = ({
  children,
  className,
  duration = 0.5,
  delayMultiple = 0.08,
  ...
  return (
    <motion.div
      className={cn("relative inline-block text-6xl font-bold", className)}
      {...props}
      style={{
    ...
    >
      <span className="relative inline-block">
        {sparkles.map((sparkle) => (
          <SparkleComp key={sparkle.id} {...sparkle} />
        ))}
        <div className="flex">
